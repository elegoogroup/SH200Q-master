...
{
	i2cdev.readBytes(driver_Addr, start_Addr, number_Bytes, read_Buffer);
}
...
{
	i2cdev.writeBytes(driver_Addr, start_Addr, number_Bytes, write_Buffer);
}
...
void sh200q_init(void)
{
	...
	Wire.begin();
	Wire.beginTransmission(SH200Q_ADDRESS);
	if (Wire.endTransmission() != 0)
		Serial.println("sh200q_init(): I2C failed.");
